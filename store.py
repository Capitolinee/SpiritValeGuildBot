...
class SheetsStore:
    ...
    def upsert_job(self, name: str, tier: int, parent: str, image: str) -> str:
        ...
        for r in rows:
            if r.get("職業名稱", "").strip() == name:
                ...
                return "updated"
        row_num = self.find_first_empty_row(SHEET_JOBS, key_col_index=1)
    ...
    def upsert_character(self, discord_id: str, display_name: str, char_name: str, job: str, position: str = "") -> str:
        ...
        for r in rows:
            if r.get("Discord ID", "").strip() == str(discord_id) and normalize_name(r.get("角色名稱", "")) == key:
                ...
                return "updated"
        row_num = self.find_first_empty_row(SHEET_CHARACTERS, key_col_index=1)
    ...
    def ensure_account_row(self, discord_id: str, display_name: str):
        ...
        for r in self.get_rows(SHEET_ACCOUNTS):
            if r.get("Discord ID", "").strip() == str(discord_id):
                return
        row_num = self.find_first_empty_row(SHEET_ACCOUNTS, key_col_index=1)
    ...
    def append_item_rows(self, session_id, when_iso, members, item_name, item_index, item_type, contributor):
        ...
        """
        sheet = SHEET_SESSIONS
        if item_type == "分潤":
            for m in members:
                row_num = self.find_first_empty_row(sheet, key_col_index=2)  # 用日期時間欄判斷空列
                self.write_row(sheet, row_num, [
            ...
            row_num = self.find_first_empty_row(sheet, key_col_index=2)
            self.write_row(sheet, row_num, [
    ...
    def sell_item(self, session_id: str, item_index: int, amount: int) -> dict:
        ...
        if item_type == "分潤":
            ...
            for r in target_rows:
                # J售出金額 K均分$$ L已領 M領取時間 -> 欄位 10,11,12,13
                self.write_row(SHEET_SESSIONS, r["_row"], [amount, round(per_person, 2), False, ""], start_col=10)
        else:
            per_person = amount
            for r in target_rows:
                self.write_row(SHEET_SESSIONS, r["_row"], [amount, "", "", ""], start_col=10)
    ...
    def claim_for_user(self, discord_id: str, session_id: str = None) -> dict:
        ...
        for r in self.get_rows(SHEET_SESSIONS):
            ...
            if already or not sale or not per_person:
                continue
            self.write_row(SHEET_SESSIONS, r["_row"], [True, now], start_col=12)
